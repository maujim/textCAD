...
describe('ChatWindow', () => {
  ...
  it('submits messages and processes AI response', async () => {
    ...
    await waitFor(() => {
      expect(mockProps.onUpdateModel).toHaveBeenCalledWith('add_hole', { radius: 5 });
      expect(mockProps.onUpdateReasoning).toHaveBeenCalledWith('Adding a hole with radius 5');
    });
  });
